...
public class EditGrade extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EditGrade</title>");
            out.println("</head>");
            out.println("<body>");
            String sid = request.getParameter("id");
            int id = Integer.parseInt(sid);
            Grade e = GradeDao.getGradeById(id);
            out.print("<form action='EditGrade2' method='post'>");
            out.print("<table>");
            out.print("<tr><td>Id</td><td><input type='text' name='id' value='" + e.getId() + "'/></td></tr>");
            out.print("<tr><td>GradeId:</td><td><input type='text' name='gradeId' value='" + e.getGradeId() + "'/></td></tr>");
